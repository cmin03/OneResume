...
const ResumeForm = ({
  ...
  const renderStepNavigation = () => {
    ...
    return (
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-12 mt-12 border-t border-zinc-500/10">
          ...
          {isFirst ? (
            ...
            <button 
              ...
              className={`w-full py-4 rounded-2xl font-black text-sm transition-all border ${
                isDarkMode ? 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:text-white' : 'bg-gray-50 border-gray-200 text-zinc-500 hover:text-zinc-800 shadow-sm'
              }`}
            >
              ← 이전 단계
            </button>
        ...
        <div className="flex-1 w-full">
          {!isLast ? (
            <button 
              type="button" 
              onClick={() => { setActiveTab(tabs[activeIndex + 1].id); scrollToTop(); }}
              className="w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-2xl font-black shadow-xl shadow-blue-600/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 group text-sm lg:text-base"
            >
              다음 단계: {tabs[activeIndex + 1].label}
  ...
  const YearMonthPicker = ({ value, onChange, isDarkMode, placeholder = "선택" }) => {
    ...
    const triggerClass = `px-4 py-3 rounded-xl border outline-none transition-all cursor-pointer text-[14px] font-bold flex items-center justify-between group ${isDarkMode ? "bg-zinc-900 border-zinc-700 text-zinc-100 hover:border-zinc-500" : "bg-white border-gray-200 text-zinc-900 hover:border-gray-400"}`;
    const listClass = `absolute bottom-[calc(100%+8px)] left-0 right-0 z-[120] max-h-60 overflow-y-auto rounded-2xl border-2 shadow-2xl custom-scrollbar animate-in fade-in slide-in-from-bottom-2 duration-200 ${isDarkMode ? "bg-zinc-900 border-zinc-700 shadow-black/50" : "bg-white border-zinc-100 shadow-zinc-200/50"}`;
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-4 lg:p-8">
        <div className="max-w-[1400px] mx-auto space-y-8">
          <DragDropContext onDragStart={onDragStart} onDragEnd={handleDragEnd}>
                  ...
                  <div className="flex flex-col md:flex-row gap-10 items-start">
                    <div className="shrink-0 mx-auto md:mx-0 text-center group">
                      <div className="w-[120px] h-[160px] rounded-2xl overflow-hidden border-2 shadow-xl bg-white dark:bg-zinc-900 border-white dark:border-zinc-700 relative">
                        {formData.profileImageUrl ? <img src={formData.profileImageUrl} alt="프로필" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-zinc-300"><svg className="h-16 w-16" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg></div>}
                      </div>
                      <label className="mt-4 inline-block cursor-pointer px-4 py-1.5 rounded-full bg-blue-50 dark:bg-blue-900/30 transition-colors hover:bg-blue-100 dark:hover:bg-blue-900/50"><span className="text-[11px] font-black uppercase text-blue-600 dark:text-blue-400">사진 변경</span><input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} /></label>
                    </div>
                    <div className="flex-1 w-full grid gap-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        <div className="flex flex-col gap-2"><label className={`pl-1 text-[12px] font-black uppercase tracking-wider ${theme.labelText}`}>이름 *</label><input type="text" name="username" value={formData.username || ""} onChange={handleChange} placeholder="홍길동" className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg}`} /></div>
                        <div className="flex flex-col gap-2"><label className={`pl-1 text-[12px] font-black uppercase tracking-wider ${theme.labelText}`}>성별</label><div className={`relative flex p-1 rounded-xl border ${theme.innerInputBg} h-[48px]`}><div className={`absolute top-1 bottom-1 w-[calc(50%-4px)] rounded-lg transition-all duration-500 shadow-sm ${formData.gender === 'male' ? 'left-1 bg-blue-600' : (formData.gender === 'female' ? 'left-[calc(50%)] bg-pink-500' : 'opacity-0')}`} /><button type="button" onClick={() => handleChange({ target: { name: 'gender', value: 'male' } })} className={`relative z-10 flex-1 flex items-center justify-center text-[11px] font-black ${formData.gender === 'male' ? 'text-white' : 'text-zinc-500'}`}>남성</button><button type="button" onClick={() => handleChange({ target: { name: 'gender', value: 'female' } })} className={`relative z-10 flex-1 flex items-center justify-center text-[11px] font-black ${formData.gender === 'female' ? 'text-white' : 'text-zinc-500'}`}>여성</button></div></div>
                        <div className="flex flex-col gap-2"><div className="flex items-center justify-between pl-1"><label className={`text-[12px] font-black uppercase tracking-wider ${theme.labelText}`}>나이</label><label className="flex items-center gap-2 cursor-pointer group"><input type="checkbox" name="useInternationalAge" checked={formData.useInternationalAge || false} onChange={handleChange} className="sr-only peer" /><div className={`w-4.5 h-4.5 rounded-md border-2 transition-all duration-200 flex items-center justify-center ${isDarkMode ? 'bg-zinc-800 border-zinc-700 peer-checked:bg-blue-600 peer-checked:border-blue-600' : 'bg-white border-zinc-300 peer-checked:bg-blue-600 peer-checked:border-blue-600'}`}><svg className={`w-3 h-3 text-white transition-transform duration-200 ${formData.useInternationalAge ? 'scale-100' : 'scale-0'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={4}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg></div><span className={`text-[11px] font-black transition-colors ${isDarkMode ? 'text-zinc-300 group-hover:text-zinc-100' : 'text-zinc-500 group-hover:text-zinc-700'}`}>만 나이</span></label></div><input type="number" name="age" value={formData.age || ""} onChange={handleChange} placeholder="숫자" className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg}`} /></div>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5"><div className="flex flex-col gap-2"><label className={`pl-1 text-[12px] font-black uppercase tracking-wider ${theme.labelText}`}>전화번호</label><input type="text" name="phone" value={formData.phone || ""} onChange={handleChange} placeholder="010-0000-0000" className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg}`} /></div><div className="flex flex-col gap-2"><label className={`pl-1 text-[12px] font-black uppercase tracking-wider ${theme.labelText}`}>이메일</label><input type="email" name="email" value={formData.email || ""} onChange={handleChange} placeholder="example@email.com" className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg}`} /></div></div>
                      <div className="flex flex-col gap-2 pt-2"><div className="flex justify-between items-center pl-1"><label className={`text-[12px] font-black uppercase tracking-wider ${theme.labelText}`}>한 줄 소개</label><button type="button" onClick={() => handleAiAudit('bio', formData.bio)} className="text-[10px] font-black text-blue-600 dark:text-blue-400 hover:underline">AI 컨설팅</button></div><textarea name="bio" value={formData.bio || ""} onChange={handleChange} onInput={autoExpand} rows="1" placeholder="나를 표현하는 핵심 문장" className={`w-full px-4 py-3 rounded-xl border outline-none resize-none transition-all ${theme.innerInputBg} leading-relaxed min-h-[48px] overflow-hidden`} /></div>
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-1 gap-6">
                  <div className={`p-6 lg:p-8 rounded-[32px] border ${theme.cardBg} space-y-5`}><div className="flex items-center gap-2 mb-2"><div className="w-1.5 h-4 bg-blue-600 rounded-full" /><label className={`text-[13px] font-black uppercase tracking-widest ${theme.labelText}`}>거주지 정보</label></div><div className="flex gap-3"><input type="text" value={formData.address || ""} readOnly onClick={() => setIsAddressOpen(true)} placeholder="주소 검색" className={`flex-1 px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg} cursor-pointer`} /><button type="button" onClick={() => setIsAddressOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white font-black px-8 rounded-xl text-xs shadow-lg shadow-blue-600/20 hover:scale-[1.02] active:scale-[0.98] transition-all">주소 검색</button></div><input type="text" name="addressDetail" value={formData.addressDetail || ""} onChange={handleChange} placeholder="상세주소 입력" className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg}`} /></div>
                  <div className={`p-6 lg:p-8 rounded-[32px] border ${theme.cardBg} space-y-5`}>
                    ...
                    <div className={`grid grid-cols-1 gap-5 ${!['미필', '해당없음', '면제', ''].includes(formData.militaryStatus) ? 'md:grid-cols-3' : (formData.militaryStatus === '면제' ? 'md:grid-cols-2' : 'md:grid-cols-1')}`}>
                      ...
                      {formData.militaryStatus === '면제' && <div className="flex flex-col gap-2"><label className={`pl-1 text-[11px] font-black uppercase ${theme.subText}`}>면제 사유</label><input type="text" name="militaryExemption" value={formData.militaryExemption || ""} onChange={handleChange} placeholder="면제 사유를 간단히 입력" className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg}`} /></div>}
                      {!['미필', '해당없음', '면제', ''].includes(formData.militaryStatus) && (<><div className="flex flex-col gap-2"><label className={`pl-1 text-[11px] font-black uppercase ${theme.subText}`}>군별</label><input type="text" name="militaryBranch" value={formData.militaryBranch || ""} onChange={handleChange} placeholder="육군" className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg}`} /></div><div className="flex flex-col gap-2"><label className={`pl-1 text-[11px] font-black uppercase ${theme.subText}`}>계급</label><input type="text" name="militaryRank" value={formData.militaryRank || ""} onChange={handleChange} placeholder="병장" className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg}`} /></div></>)}
                    </div>
                    {!['미필', '해당없음', '면제', ''].includes(formData.militaryStatus) && (<div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-2 border-t border-zinc-500/10 mt-5"><div className="flex flex-col gap-2"><label className={`pl-1 text-[11px] font-black uppercase ${theme.subText}`}>입대년월</label><YearMonthPicker value={formData.militaryStartDate || ""} onChange={(val) => handleChange({ target: { name: 'militaryStartDate', value: val } })} isDarkMode={isDarkMode} /></div><div className="flex flex-col gap-2"><label className={`pl-1 text-[11px] font-black uppercase ${theme.subText}`}>전역년월</label><YearMonthPicker value={formData.militaryEndDate || ""} onChange={(val) => handleChange({ target: { name: 'militaryEndDate', value: val } })} isDarkMode={isDarkMode} /></div></div>)}
                  </div>
                </div>
              ...
              <div className="space-y-8 animate-fade-in">
                <div className={`p-6 lg:p-8 rounded-[32px] border ${theme.cardBg} space-y-6`}>
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between pl-1"><div className="flex items-center gap-2"><label className={`text-[12px] font-black uppercase tracking-wider ${theme.labelText}`}>서브도메인</label><div className="group relative"><svg className="w-4 h-4 text-zinc-400 cursor-help" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg><div className={`absolute left-0 top-full mt-3 w-72 p-5 rounded-[24px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[100] shadow-2xl leading-relaxed border ${isDarkMode ? 'bg-zinc-900 text-white border-white/10 shadow-blue-500/20' : 'bg-white text-zinc-800 border-zinc-200 shadow-zinc-400/20'}`}><div className={`absolute -top-1.5 left-4 w-3 h-3 rotate-45 border-t border-l ${isDarkMode ? 'bg-zinc-900 border-white/10' : 'bg-white border-zinc-200'}`}></div><div className="flex items-center gap-2 mb-2 text-blue-500 font-black text-[12px] relative z-10"><span>🌐</span><span>나만의 이력서 주소 설정</span></div><p className={`font-medium mb-3 relative z-10 ${isDarkMode ? 'text-zinc-300' : 'text-zinc-600'}`}>인사담당자나 친구들에게 공유할 <span className={`font-bold underline decoration-blue-500 ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>웹사이트 주소</span>를 직접 정할 수 있어요.</p><div className={`p-3 rounded-xl border relative z-10 ${isDarkMode ? 'bg-white/5 border-white/5' : 'bg-zinc-50 border-zinc-100'}`}><p className={`text-[11px] mb-1 font-bold italic ${isDarkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>설정 예시:</p><p className="text-[12px] font-black tracking-tight">아이디에 <span className="text-blue-500">"dev-hong"</span> 입력 시<br/><span className="text-blue-500 underline">https://dev-hong.oneresume.kr</span></p></div></div></div></div></div>
                    <div className="flex items-center"><input type="text" name="subdomain" value={formData.subdomain || ""} onChange={handleChange} placeholder="your-id" className={`flex-1 px-4 py-3 border rounded-l-xl outline-none border-r-0 ${theme.innerInputBg} font-black text-base`} /><span className={`px-5 py-3 font-black text-[13px] border border-l-0 rounded-r-xl ${isDarkMode ? 'bg-zinc-800 border-zinc-700 text-zinc-400' : 'bg-gray-100 border-gray-200 text-zinc-500'}`}>.oneresume.kr</span></div>
                ...
                <div className={`p-6 lg:p-8 rounded-[32px] border ${theme.cardBg} space-y-8`}><div className="flex items-center gap-2 mb-2"><div className="w-1.5 h-4 bg-blue-600 rounded-full" /><h4 className={`text-[13px] font-black uppercase tracking-widest ${theme.labelText}`}>외부 링크 연동</h4></div><div className="flex flex-col gap-3"><label className={`pl-1 text-[12px] font-black uppercase tracking-wider ${theme.labelText}`}>GitHub 주소</label><div className="flex gap-3"><input type="text" name="githubUrl" value={formData.githubUrl || ""} onChange={handleChange} placeholder="https://github.com/your-id" className={`flex-1 px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg}`} />{formData.githubUrl && <button type="button" onClick={handleGithubSync} className="bg-zinc-900 dark:bg-blue-600 text-white font-black px-6 rounded-xl text-xs hover:scale-[1.02] active:scale-[0.98] transition-all">동기화</button>}</div></div><div className="flex flex-col gap-3"><label className={`pl-1 text-[12px] font-black uppercase tracking-wider ${theme.labelText}`}>기술 블로그</label><input type="text" name="blogUrl" value={formData.blogUrl || ""} onChange={handleChange} placeholder="https://velog.io/@your-id" className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg}`} /></div></div>
                {renderStepNavigation()}
              </div>
            )}
            ...
            {activeTab === 'edu' && (
              <div className="space-y-8 animate-fade-in">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className={`p-6 lg:p-8 rounded-[32px] border ${theme.cardBg} space-y-6`}><h4 className={`text-[13px] font-black uppercase tracking-widest ${theme.labelText} mb-2`}>학력 사항</h4><div className="relative flex flex-col gap-2"><label className={`pl-1 text-[12px] font-black uppercase tracking-wider ${theme.labelText}`}>학교명</label><input type="text" name="school" value={formData.school || ""} onChange={(e) => { handleChange(e); searchSchool(e.target.value); }} onFocus={() => (formData?.school || "").length >= 2 && setShowSchoolList(true)} onBlur={() => setTimeout(() => setShowSchoolList(false), 200)} autoComplete="off" placeholder="학교명 입력 (2자 이상)" className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg}`} />{showSchoolList && (<div className={`absolute top-[calc(100%+8px)] left-0 right-0 z-50 max-h-60 overflow-y-auto rounded-2xl border-2 shadow-2xl ${isDarkMode ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-zinc-100'}`}>{schoolResults.length > 0 ? schoolResults.map((item, idx) => (<div key={idx} onClick={() => { handleChange({ target: { name: 'school', value: item.schoolName } }); setShowSchoolList(false); }} className={`px-5 py-3 cursor-pointer border-b last:border-0 transition-colors ${isDarkMode ? 'border-zinc-800 hover:bg-zinc-800' : 'border-zinc-50 hover:bg-blue-50'}`}><div className={`font-black text-[14px] ${isDarkMode ? 'text-white' : 'text-zinc-800'}`}>{item.schoolName}</div><div className={`text-[11px] mt-0.5 ${isDarkMode ? 'text-zinc-400' : 'text-zinc-500 opacity-70'}`}>{item.region} | {item.campusName}</div></div>)) : <div className={`p-5 text-center text-xs font-bold ${isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>결과가 없습니다.</div>}</div>)}</div><div className="relative flex flex-col gap-2"><label className={`pl-1 text-[12px] font-black uppercase tracking-wider ${theme.labelText}`}>전공명</label><input type="text" name="major" value={formData.major || ""} onChange={(e) => { handleChange(e); searchMajor(e.target.value); }} onFocus={() => (formData?.major || "").length >= 2 && setShowMajorList(true)} onBlur={() => setTimeout(() => setShowMajorList(false), 200)} autoComplete="off" placeholder="전공명 입력" className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${theme.innerInputBg}`} />{showMajorList && (<div className={`absolute top-[calc(100%+8px)] left-0 right-0 z-50 max-h-60 overflow-y-auto rounded-2xl border-2 shadow-2xl ${isDarkMode ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-zinc-100'}`}>{majorResults.length > 0 ? majorResults.map((item, idx) => { const name = item.majorName || item.mClass || item.facilName || ""; return name && <div key={idx} onClick={() => { handleChange({ target: { name: 'major', value: name } }); setShowMajorList(false); }} className={`px-5 py-4 font-black cursor-pointer border-b last:border-0 transition-colors ${isDarkMode ? 'text-white border-zinc-800 hover:bg-zinc-800' : 'text-zinc-800 border-zinc-50 hover:bg-blue-50'}`}>{name}</div>; }) : <div className={`p-5 text-center text-xs font-bold ${isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>결과가 없습니다.</div>}</div>)}</div><div className="flex flex-col gap-2"><label className={`pl-1 text-[12px] font-black uppercase tracking-wider ${theme.labelText}`}>학점 (GPA)</label><input type="text" name="gpa" value={formData.gpa || ""} onChange={handleChange} placeholder="4.5 / 4.5" className={`w-full px-4 py-3 rounded-xl border transition-all ${theme.innerInputBg}`} /></div></div>
              ...
              <div className="space-y-8 animate-fade-in">
                ...
                <div className="flex flex-col gap-6">
                  {formData.workExperiences.map((work, index) => (
                    <div key={index} className={`p-6 lg:p-8 rounded-[32px] border ${theme.cardBg} relative group space-y-6 transition-all hover:border-blue-500/30 shadow-sm`}>
                      <button type="button" onClick={() => removeWork(index)} className="absolute -right-2 -top-2 w-9 h-9 bg-red-500 text-white rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all font-bold shadow-xl z-10">✕</button>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5"><div className="flex flex-col gap-2"><label className={`text-[11px] font-black uppercase ${theme.subText}`}>회사명</label><input type="text" name="companyName" value={work.companyName || ""} onChange={(e) => handleWorkChange(index, e)} placeholder="회사명 입력" className={`w-full px-4 py-3 rounded-xl border ${theme.innerInputBg} font-black`} /></div><div className="flex flex-col gap-2"><label className={`text-[11px] font-black uppercase ${theme.subText}`}>직급 / 직위</label><input type="text" name="position" value={work.position || ""} onChange={(e) => handleWorkChange(index, e)} placeholder="예: 과장, 선임, 사원" className={`w-full px-4 py-3 rounded-xl border ${theme.innerInputBg}`} /></div></div>
              ...
              <div className="space-y-8 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {formData.certifications.map((cert, index) => (
                    <div key={index} className={`p-6 lg:p-8 rounded-[32px] border ${theme.cardBg} relative group space-y-5 transition-all hover:border-blue-500/30 shadow-sm`}><button type="button" onClick={() => removeCert(index)} className="absolute -right-2 -top-2 w-9 h-9 bg-red-500 text-white rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all font-bold shadow-xl z-10">✕</button>
                      ...
                      <div className="grid grid-cols-2 gap-4"><div className="flex flex-col gap-2"><label className={`text-[11px] font-black uppercase ${theme.subText}`}>발급기관</label><input type="text" name="issuer" value={cert.issuer || ""} onChange={(e) => handleCertChange(index, e)} className={`w-full px-4 py-3 rounded-xl border ${theme.innerInputBg} text-[14px]`} /></div><div className="flex flex-col gap-2"><label className={`text-[11px] font-black uppercase ${theme.subText}`}>점수 / 등급</label><input type="text" name="score" value={cert.score || ""} onChange={(e) => handleCertChange(index, e)} className={`w-full px-4 py-3 rounded-xl border ${theme.innerInputBg} text-[14px]`} /></div></div>
                      <div className="flex flex-col gap-2"><label className={`text-[11px] font-black uppercase ${theme.subText}`}>취득일</label><YearMonthPicker value={cert.date || ""} onChange={(val) => handleCertChange(index, { target: { name: 'date', value: val } })} isDarkMode={isDarkMode} /></div>
                    </div>
                  ))}
                </div>
                {renderStepNavigation()}
                  ...
                  <div {...provided.droppableProps} ref={provided.innerRef} className="flex flex-col gap-6">
                    {formData.projects.map((project, index) => (
                      <Draggable key={project.id} draggableId={String(project.id)} index={index}>{(provided, snapshot) => (
                        <div ref={provided.innerRef} {...provided.draggableProps} style={provided.draggableProps.style} className={`p-6 lg:p-8 rounded-[32px] border transition-all duration-200 ${snapshot.isDragging ? 'shadow-2xl border-blue-500 bg-white dark:bg-zinc-800 z-[100] scale-[1.02]' : theme.cardBg} relative group shadow-sm`}><button type="button" onClick={() => removeProject(index)} className="absolute -right-2 -top-2 w-9 h-9 bg-red-500 text-white rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all font-bold shadow-xl z-50">✕</button><div {...provided.dragHandleProps} className="absolute left-1/2 -translate-x-1/2 top-3 w-12 h-1.5 bg-zinc-300 dark:bg-zinc-700 rounded-full cursor-grab active:cursor-grabbing hover:bg-blue-400 transition-colors" />
                          <div className="space-y-6 mt-4">
                            <div className="flex flex-col gap-2"><label className={`text-[11px] font-black uppercase ${theme.subText}`}>프로젝트명</label><input type="text" name="name" value={project.name || ""} onChange={(e) => handleProjectChange(index, e)} className={`w-full px-4 py-3 rounded-xl border ${theme.innerInputBg} font-black text-lg`} /></div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5"><div className="flex flex-col gap-2"><label className={`text-[11px] font-black uppercase ${theme.subText}`}>담당 역할</label><input type="text" name="role" value={project.role || ""} onChange={(e) => handleProjectChange(index, e)} autoComplete="off" placeholder="예: 프론트엔드 개발" className={`w-full px-4 py-3 rounded-xl border ${theme.innerInputBg}`} /></div><div className="flex flex-col gap-2"><label className={`text-[11px] font-black uppercase ${theme.subText}`}>기술 스택</label><input type="text" name="techStack" value={project.techStack || ""} onChange={(e) => handleProjectChange(index, e)} placeholder="예: React, Node.js" className={`w-full px-4 py-3 rounded-xl border ${theme.innerInputBg}`} /></div></div>
                            <div className="flex flex-col gap-2"><label className={`text-[11px] font-black uppercase ${theme.subText}`}>진행 기간</label><PeriodPicker value={project.period || ""} onChange={(val) => handleProjectChange(index, { target: { name: 'period', value: val } })} isDarkMode={isDarkMode} /></div>
                            <div className="flex flex-col gap-2"><div className="flex justify-between items-center"><label className={`text-[11px] font-black uppercase ${theme.subText}`}>상세 성과</label><button type="button" onClick={() => handleAiAudit("project-" + index, project.description)} className="text-[10px] text-blue-600 font-black hover:underline px-2 py-1 bg-blue-50 dark:bg-blue-900/20 rounded-lg">AI 분석 리포트</button></div><textarea name="description" value={project.description || ""} onChange={(e) => handleProjectChange(index, e)} onInput={autoExpand} rows="4" className={`w-full px-4 py-3 rounded-xl border ${theme.innerInputBg} resize-none leading-relaxed text-[14px] min-h-[120px] overflow-hidden`} /></div>
                          </div>
                        </div>
                      )}</Draggable>
            ...
            {activeTab === 'extra' && (
              <div className="space-y-8 animate-fade-in">
                <div className={`p-6 lg:p-8 rounded-[32px] border ${theme.cardBg} space-y-8`}><div className="flex flex-col gap-3 relative"><div className="flex justify-between items-center pl-1"><label className={`text-[13px] font-black uppercase tracking-wider ${theme.labelText}`}>성장과정</label><button type="button" onClick={() => handleAiAudit('selfIntroGrowth', formData.selfIntroGrowth)} className="text-[10px] font-black text-blue-600 dark:text-blue-400 hover:underline">AI 컨설팅</button></div><textarea name="selfIntroGrowth" value={formData.selfIntroGrowth || ""} onChange={handleChange} onInput={autoExpand} rows="6" className={`w-full px-5 py-4 rounded-2xl border outline-none transition-all ${theme.innerInputBg} leading-relaxed min-h-[150px] overflow-hidden`} /></div><div className="flex flex-col gap-3 relative"><div className="flex justify-between items-center pl-1"><label className={`text-[13px] font-black uppercase tracking-wider ${theme.labelText}`}>성격의 장단점</label><button type="button" onClick={() => handleAiAudit('selfIntroCharacter', formData.selfIntroCharacter)} className="text-[10px] font-black text-blue-600 dark:text-blue-400 hover:underline">AI 컨설팅</button></div><textarea name="selfIntroCharacter" value={formData.selfIntroCharacter || ""} onChange={handleChange} onInput={autoExpand} rows="6" className={`w-full px-5 py-4 rounded-2xl border outline-none transition-all ${theme.innerInputBg} leading-relaxed min-h-[150px] overflow-hidden`} /></div><div className="flex flex-col gap-3 relative"><div className="flex justify-between items-center pl-1"><label className={`text-[13px] font-black uppercase tracking-wider ${theme.labelText}`}>지원동기 및 포부</label><button type="button" onClick={() => handleAiAudit('selfIntroMotivation', formData.selfIntroMotivation)} className="text-[10px] font-black text-blue-600 dark:text-blue-400 hover:underline">AI 컨설팅</button></div><textarea name="selfIntroMotivation" value={formData.selfIntroMotivation || ""} onChange={handleChange} onInput={autoExpand} rows="6" className={`w-full px-5 py-4 rounded-2xl border outline-none transition-all ${theme.innerInputBg} leading-relaxed min-h-[150px] overflow-hidden`} /></div></div>
                {renderStepNavigation()}
              </div>
            )}
          </DragDropContext>
      ...
      {activeIndex !== 0 && activeIndex !== tabs.length - 1 && (
        <div className="flex-none p-5 lg:p-6 border-t border-zinc-700/10 bg-zinc-500/5 backdrop-blur-md">
          <form onSubmit={handleSubmit}>
            <button 
              type="submit" 
              className="w-full py-4 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-black rounded-2xl shadow-xl shadow-emerald-500/20 text-sm lg:text-base transition-all active:scale-[0.98] hover:scale-[1.02]"
            >
              이력서 저장
            </button>
          </form>
